...
d = 365.25 * 5 # Length of award period in days.
...
xrange = np.arange(0, 1000000, 1, dtype=int) # Could be something like myRange = range(1,1000,1)
...
clrsB = ['grey' if x == 1000 #(selected / factor) 
        else 'red' if x < 1000# (selected / factor) 
        else 'green'
        for x in xrange]
...
####
##fig, ax = plt.subplots()
##plt.suptitle('Assuming: ' + str(n) + ' beacon validators; NO operating ' +str(Aminipools) + ' minipools; ' +str(d)+ ' d validating period.')
...
upperclip = int(Bmean + 1000)
dfB = df.head(upperclip)
dfB = dfB.tail(2000)
...
##
#axA = sns.barplot(x='x', y='bDataA', data=df2, label=str(Aminipools) + ' minipool(s)', palette=clrsA)
axB = sns.barplot(x='x', y='bDataB', data=dfB, label=str(Bminipools) + ' minipool(s)', palette=clrsB, alpha = 0.5)
#sns.lineplot(x='x', y='bCDF_A', data=df, label=str(Aminipools) + ' minipool(s)', palette=clrsA)
